...
// Model for Users entity
    models.Users = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/sportzal-rest/webresources/ua.at.programmers.entities.users/",
        idAttribute: 'userId',
        defaults: {
            userLogin: "",
            userPasswd: "",
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/sportzal-rest/webresources/ua.at.programmers.entities.users/';
            }
    ...
    // Collection class for Users entities
    models.UsersCollection = Backbone.Collection.extend({
        model: models.Users,
        url: "http://localhost:8080/sportzal-rest/webresources/ua.at.programmers.entities.users/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.UsersCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-users-list-itemis template identifier for item
                        templateName: '#tpl-users-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.users = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.users,
                    // tpl-users-details is template identifier for chosen model element
                    templateName: '#tpl-users-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Users();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-users-details is a template identifier for chosen model element
                templateName: '#tpl-users-details',
